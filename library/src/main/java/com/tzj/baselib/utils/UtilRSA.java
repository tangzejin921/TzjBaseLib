...
public class UtilRSA {
    ...
    private static String sign(byte[] data, String privateKey) throws Exception {
        byte[] keyBytes = UtilCipher.Base64.decode(privateKey);
        ...
        return UtilCipher.Base64.encode(signature.sign());
    }
    ...
    private static boolean verify(byte[] data, String publicKey, String sign)
            throws Exception {
        byte[] keyBytes = UtilCipher.Base64.decode(publicKey);
        ...
        signature.update(data);
        return signature.verify(UtilCipher.Base64.decode(sign));
    }
    ...
    public static byte[] decryptByPrivateKey(byte[] encryptedData, String privateKey)
        ...
        byte[] keyBytes = UtilCipher.Base64.decode(privateKey);
        PKCS8EncodedKeySpec pkcs8KeySpec = new PKCS8EncodedKeySpec(keyBytes);
        KeyFactory keyFactory = getKeyFactory(KEY_ALGORITHM);
    ...
    public static String decryptByPrivateKey(String content, String privateKey, String charset)
            throws Exception {
        byte[] encryptedData = UtilCipher.Base64.decode(content);
        byte[] bResult = decryptByPrivateKey(encryptedData, privateKey);
    ...
    public static String decryptByPublicKey(String content, String publicKey, String charset) throws Exception {
        byte[] encryptedData = UtilCipher.Base64.decode(content);
        byte[] bResult = decryptByPublicKey(encryptedData, publicKey);
        return new String(bResult, charset);
    }

    @SuppressWarnings("unused")
    private static byte[] getBytes(byte[] src, int offset, int len) {
    ...
    public static String encryptByPublicKey(String content, String publicKey, String charset) throws Exception {
        byte[] data = content.getBytes(charset);
        byte[] bResult = encryptByPublicKey(data, publicKey);
        return UtilCipher.Base64.encode(bResult);
    }
...
//        return encode(bResult);
    ...
    private static KeyFactory getKeyFactory(String name) throws NoSuchAlgorithmException {
        return KeyFactory.getInstance("RSA/ECB/PKCS1Padding"/*, new org.bouncycastle.jce.provider.BouncyCastleProvider()*/);
    }
}